...
export const addReview = async (req,res) => {
  ...
  const query = "INSERT INTO reviews (rating,review_text,user_id,game_id) VALUES ($1,$2,$3,$4) RETURNING *;";
...
export const deleteReview = async (req,res) => {
  ...
  const query = "DELETE FROM reviews WHERE game_id = $1 AND user_id = $2 RETURNING *;";

  const result = await pool.query(query,[gameId,userId]);

  if (result.rowCount === 0){
    throw new NotFoundError(`No review for game with id ${gameId} for current user`);
  }
...
export const getGameReviews = async (req,res) => {
  ...
  const countQuery = "SELECT CAST(COUNT(*) AS INT) FROM reviews WHERE game_id = $1 AND review_text IS NOT NULL;";

  const count = await pool.query(countQuery,[gameId]);
  
  res.status(StatusCodes.OK).send({reviews:reviews.rows, review_count:count.rows[0].count, last_page: Math.ceil(count.rows[0].count / limit)});
};
